...
const FIXTURE =
  'qinglong/prompt-output-key-rotation-kubernetes-live-contract@v1';
...
function validatePromptOutputKeyRotationKubernetesLiveReport(report) {
  const findings = [];
  if (
    !exactKeys(report, [
      'database',
    ...
    findings.push('QL3_PROMPT_OUTPUT_KEY_ROTATION_LIVE_REPORT_SHAPE');
  }
  if (
    ...
    findings.push('QL3_PROMPT_OUTPUT_KEY_ROTATION_LIVE_PLATFORM');
  }
  if (
    ...
    findings.push('QL3_PROMPT_OUTPUT_KEY_ROTATION_LIVE_DATABASE');
  }
  if (
    ...
    report?.operation?.generationBefore !== 1 ||
    report?.operation?.generationAfter !== 2 ||
    report?.operation?.keyCountAfter !== 2 ||
    ...
    report?.operation?.stagedFileMode !== 0o440 ||
    typeof report?.operation?.transientUnavailableObserved !== 'boolean' ||
    ![
      'atomicWriterSymlink',
      'contentFreeFacts',
      'denyCanaryControlReachable',
    ...
    findings.push('QL3_PROMPT_OUTPUT_KEY_ROTATION_LIVE_OPERATION');
  }
  if (
    ...
    findings.push('QL3_PROMPT_OUTPUT_KEY_ROTATION_LIVE_GATES');
  }
  if (
    JSON.stringify(report?.limitations) !== JSON.stringify(LIMITATIONS) ||
    containsSensitiveMaterial(report)
  ) {
    findings.push('QL3_PROMPT_OUTPUT_KEY_ROTATION_LIVE_CONTENT');
  }
