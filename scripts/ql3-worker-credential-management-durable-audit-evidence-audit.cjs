...
function runCli(argv) {
  if (argv.length !== 1 || !argv[0].startsWith('--report=')) {
    throw new Error(
      'usage: ql3-worker-credential-management-durable-audit-evidence-audit --report=/absolute/report.json',
    );
  }
  const report = readReport(argv[0].slice('--report='.length));
  const audit = validateWorkerCredentialManagementDurableAuditEvidence(report);
