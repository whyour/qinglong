...
本 ADR 已由提交 `f28bf74d1bd29e9b8a8727915de19509f4bda9cf` 的远端主 CI `33236204273`、Kubernetes deployment `33236204254` 与显式 Local Console milestone `33237026187` 闭合。新 run 生成独立的 amd64/arm64 Console Trial Kit 与 milestone index；下载索引通过 `SHA256SUMS` 和仓库离线 auditor，返回 `compatible=true`。因此 D-422 已升级为新的可下载阶段产物；D-421 archive 仅保留为不含 Web 编辑能力的历史实物，没有被改名复用。该结论不把 Alpha 扩大为公开 release、2.x 生产升级或长期支持版本。
