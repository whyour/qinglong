...
| D-426c2 受审核 Automation apply/rollback | Trial Kit v10 的 exact amd64/arm64 headless 实物与 milestone v6 已交付；三阶段脚本只消费外部 owner-private decision，贯通 capture→437 条 review decision→1 条 Automation decision→1 Task/1 Trigger apply/verify→explicit rollback/verify，保持 128 MiB/0.5 CPU/32 PID、无网络、只读 rootfs | 不应用 Secret/Config、Run History，不执行 completion/restart，不是 2.x 老面板 compatibility layer 或生产升级包 |
