...
export interface MigrationStreamManifestStep {
  id: string;
  checksum: string;
}

export interface MigrationStreamStep<TContext>
  extends MigrationStreamManifestStep {
...
function validateStream(
  stream: MigrationStreamManifest,
  executable: boolean,
): void {
  ...
  for (const migration of stream.migrations) {
    ...
    const manifestShape =
      objectShape && exactKeys(migration, ['id', 'checksum']);
    const definitionShape =
      objectShape &&
      exactKeys(migration, ['id', 'checksum', 'up']) &&
      typeof (migration as Partial<MigrationStreamStep<unknown>>).up ===
        'function';
    if (
      !objectShape ||
      !(manifestShape || definitionShape) ||
      (executable && !definitionShape) ||
      !idPattern.test(migration.id) ||
...
function assertChecksum(
  ...
  migration: MigrationStreamManifestStep,
): void {
...
export function auditMigrationStreamHistory(
  ...
  for (const record of history) {
    ...
    validateRecord(record, stream, expected.migration.id);
    assertChecksum(record, expected.migration);
  ...
  for (let index = 0; index < appliedIndexes.size; index += 1) {
    if (!appliedIndexes.has(index)) {
      const missingMigration = stream.migrations[index];
      throw new MigrationStreamHistoryCorruptionError(
        missingMigration?.id ?? 'history',
      );
    }
  }
  return appliedIds;
