...
add_cron() {
    ...
    for file in $(cat $list_add); do
        ...
        if [ -f $file ]; then
            ...
            [[ -z $cron_line ]] && cron_line=$(grep "cron " $file | awk -F "cron \"" '{print $2}' | awk -F "\" script" '{print $1}' | xargs) 
...
update_raw() {
    ...
    if [[ $? -eq 0 ]]; then
        ...
        [[ -z $cron_line ]] && cron_line=$(grep "cron " $raw_file_name | awk -F "cron \"" '{print $2}' | awk -F "\" script" '{print $1}' | xargs) 
        [[ -z $cron_line ]] && cron_line="0 6 * * *"
        if [[ -z $cron_id ]]; then
