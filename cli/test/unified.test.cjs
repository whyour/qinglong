...
test('unified ql groups and task shortcut preserve help, errors and local execution boundaries', async t => {
  ...
  for (const args of [['task', 'list'], ['task', 'run', '12'], ['subscription', 'list']]) {
  ...
  const local = call('ql', ['local', 'repair-config', '--root', 'relative', '--json']);
  ...
  await fs.writeFile(path.join(root, 'data/config/config.sh'), 'no_tee=false\n');
  await fs.writeFile(path.join(root, 'data/scripts/demo.sh'), 'printf "script:%s\\n" "$1"; return 7\n');
  ...
  assert.equal(shortcut.status, 3, shortcut.stderr);
  ...
  for (const name of ['ql', 'task']) {
    const result = spawnSync(path.join(bin, name), ['--help'], { env, encoding: 'utf8' });
    assert.equal(result.status, 0, result.stderr);
    assert.match(result.stdout, /ql task/);
  }
});

test('unified public help and API commands do not load local operators', () => {
...
if(Object.keys(require.cache).some(p=>p.includes('/dist/local/'))) process.exit(2);
});`;
  const result = spawnSync(process.execPath, ['-e', code], {
    cwd: path.resolve(__dirname, '../..'),
    env: { PATH: process.env.PATH, QL_CLI_CONFIG: '/nonexistent-ql-unified-config' },
    encoding: 'utf8',
  });
  assert.equal(result.status, 0, result.stderr);
});
