...
`;
  for (const [args, expected] of [
    ...
    for (const module of result.modules) {
      ...
      assert.doesNotMatch(
        module,
        /^(local|back|preload)[/\\]|(^|[/\\])node_modules[/\\]/,
        JSON.stringify(args),
      );
    }
    if (args.includes('--help'))
      assert.ok(
        !result.modules.some((module) =>
          /^(commands|api|config)[/\\]/.test(module),
        ),
        'Help eagerly loaded command implementation',
      );
    if (args[0] === 'task' && !args.includes('--help'))
      assert.ok(result.modules.includes(path.join('commands', 'task.js')));
    if (args[0] === 'subscription')
      assert.ok(
        result.modules.includes(path.join('commands', 'subscription.js')),
      );
  }
});
