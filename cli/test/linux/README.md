...
The expanded switched-entrypoint gate now includes panel-operator.cjs for real log retention and full-panel service reload. It currently fails the final entrypoint-retention assertion: the 2.20.1 startup loader overwrites the temporary Node wrappers with original Shell links. Retention and panel health recovery pass, but persistent deployment replacement remains incomplete until command-link selection is integrated.
